...
export const RideItineraryProvider = (props) => {
    ...
    const createRideItinerary = (newRideItinerary) => {
        return fetch("https://queued-server-tv5uq.ondigitalocean.app/rideitineraries", {
            method : "POST",
            headers: {
    ...
    const deleteRideItinerary = (rideItinerary) => {
        return fetch(`https://queued-server-tv5uq.ondigitalocean.app/rideitineraries/${rideItinerary.id}`, {
            method : "DELETE",
            headers: {
                "Authorization": `Token ${localStorage.getItem("q_token")}`,
    ...
    const getRideItineraryById = (rideItineraryId) => {
        return fetch(`https://queued-server-tv5uq.ondigitalocean.app/rideitineraries/${rideItineraryId}`, {
            headers: {
                "Authorization": `Token ${localStorage.getItem("q_token")}`,
            }
        })
    ...
    const getRideItinerariesByItineraryId = (itineraryId) => {
        return fetch(`https://queued-server-tv5uq.ondigitalocean.app/rideitineraries?itinerary_id=${itineraryId}`, {
            headers: {
                "Authorization": `Token ${localStorage.getItem("q_token")}`,
            }
        })
